    ...
    if [[ -f "dnscontrol${SUFFIX}" ]]; then
        if [[ "${BUILD_OS}" == "linux" ]]; then
            ...
            if type fpm 2>/dev/null 1>&2; then
                ...
                fpm -a "${BUILD_ARCH3}" --log error -p .ci/build --prefix /usr -s dir -t rpm "${FPM_OPTIONS[@]}" "${DNSCONTROL_FILES[@]}"
            fi
        elif [[ "${BUILD_OS}" == "freebsd" ]]; then
            # create txz archive using fpm (if available)
            if type fpm 2>/dev/null 1>&2; then
                rm -Rf ./*.txz 2>/dev/null
                fpm -a "${BUILD_ARCH2}" --log error --prefix /usr/local -s dir -t freebsd "${FPM_OPTIONS[@]}" "${DNSCONTROL_FILES[@]}"
                ...
                if [[ -n "${TXZNAME}" ]]; then
                    ...
                    if tar -tf "${TXZNAME}" | grep -qE "^[a-z]"; then
                        FTMPDIR="$(mktemp -d -p .)"
                        if [[ -d "${FTMPDIR}" ]]; then
                    ...
                    mv "${TXZNAME}" ".ci/build/${TXZNAME/\.txz/_${BUILD_ARCH2}.txz}" 2>/dev/null
                fi
            fi
        fi
